...
Jangan gunakan emoji sama sekali. Gunakan layout HTML yang bersih, elegan, dan profesional. Gunakan div dengan border-left berwarna sesuai status. JAWAB HANYA HTML VALID.`;
    try {
        const raw = await callAI([{ role: 'user', content: prompt }], false, 'gpt-4o-mini');
...
async function updateDailyAIAnalysis(logs, profile, email) {
    ...
    try {
        ...
        const rawHtml = await callAI([{ role: 'user', content: prompt }], false, 'gpt-4o-mini');
...
Gunakan bahasa Indonesia santai bersahabat (kamu/lu).`;
        ...
        try {
            const aiResponse = await callAI([{ role: 'user', content: prompt }], false, 'gpt-4o-mini');
